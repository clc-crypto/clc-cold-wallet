...
const getpass = async (prompt: string) => {
...
program
    ...
    .action(async (path: string, options: any) => {
        if (fs.existsSync(savePath)) return console.log("Wallet already loaded, please logout");
        ...
        if (options.print) console.log(JSON.parse(decryptedWallet));
...
program
    ...
    .action(async (path: string) => {
        if (!fs.existsSync(savePath)) return console.log("Wallet not loaded yet, please decrypt");
        const passwd = await getpass("Enter wallet encryption password >");
        console.log();
        const passwdChk = await getpass("Retype wallet encryption password >");
        if (passwd !== passwdChk) return console.log("\nPasswords don't match!");
...
program
    ...
    .action(async (options: any) => {
        ...
        for (const coinId in wallet) {
            ...
            if (options.validate) {
                if (!coin) coin = (await (await fetch("https://clc.ix.tc/coin/" + coinId)).json()).coin;
                const transactions = coin.transactions;
                if (transactions[transactions.length - 1].holder === pub) console.log("Valid.");
...
program
    ...
    .action(async (id: any, options: any) => {
        if (!fs.existsSync(savePath)) return console.log("Wallet not loaded yet, please decrypt");
        const wallet = JSON.parse(fs.readFileSync(savePath, "utf-8"));
        if (!wallet[id]) return console.log("You do not have this coin in this wallet!");
        if (!options.confirm) {
            if (parseInt(await input("Please retype the coin id you want to delete >")) !== parseInt(id)) return console.log("Aborting!");
...
program
    ...
    .option("-v, --validate")
    .action(async (cpath: any, options: any) => {
        if (!fs.existsSync(savePath)) return console.log("Wallet not loaded yet, please decrypt");
        ...
        if (options.validate) {
            ...
            if (transactions[transactions.length - 1].holder === pub) console.log("Valid coin, adding...");
            else return console.log("Invalid coin!");
        }

        const wallet = JSON.parse(fs.readFileSync(savePath, "utf-8"));
        if (wallet[coinId]) return console.log("Coin already in wallet!");
...
program
    ...
    .action(async (id: any) => {
        if (!fs.existsSync(savePath)) return console.log("Wallet not loaded yet, please decrypt");
        const wallet = JSON.parse(fs.readFileSync(savePath, "utf-8"));
        if (!wallet[id]) return console.log("Coin not in wallet!");
...
program
    ...
    .action(async (options: any) => {
        let kp = null;
        if (!options.private) kp = ec.genKeyPair();
        else kp = ec.keyFromPrivate(options.private);
...
program
    ...
    .action(async (id: any, addr: any) => {
        if (!fs.existsSync(savePath)) return console.log("Wallet not loaded yet, please decrypt");
        const wallet = JSON.parse(fs.readFileSync(savePath, "utf-8"));

        if (!wallet[id]) return console.log("This coin is not in your wallet");
        ...
        if (res.error) console.log("Error transacting, " + res.error);
        else {
            delete wallet[id];
            fs.writeFileSync(savePath, JSON.stringify(wallet));
...
program
    ...
    .action(async (id: any, target: any, vol: any) => {
        if (!fs.existsSync(savePath)) return console.log("Wallet not loaded yet, please decrypt");
        const wallet = JSON.parse(fs.readFileSync(savePath, "utf-8"));
        ...
        if (res.error) console.log("Error merging coin, " + res.error);
...
program
    ...
    .action(async (id: any, vol: any) => {
        ...
        if (res.error) console.log("Error splitting coin, " + res.error);
        else {
            wallet[ll] = wallet[id];
            fs.writeFileSync(savePath, JSON.stringify(wallet));
